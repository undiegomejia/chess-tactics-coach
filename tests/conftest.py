...
def mock_full_explanation() -> ClaudeExplanationPayload:
    """Provide a mock full explanation for testing."""
    return ClaudeExplanationPayload(
        mistake_category="Positional Mistake",
        ...
        tactical_motifs=["Pin", "Fork"],
        strategic_factors=["Control of the center", "Piece development"],
        recommended_plan="Develop pieces and control the center.",
        confidence=0.95,
    )
...
def messages_create_mock(**kwargs):
    ...
    if any(m.get("role") == "assistant" for m in messages):
        return FakeAnthropicMessagesResponse(
            ...
            content=[
                FakeAnthropicContentBlock(
                    block_type="full_explanation",
                    full_explanation=mock_full_explanation(),
                )
            ],
        )
    return FakeAnthropicMessagesResponse(
...
def messages_parse_mock(**kwargs):
    messages = kwargs.get("messages", [])
    if any(m.get("role") == "assistant" for m in messages):
        return SimpleNamespace(
            stop_reason="end_turn", parsed_output=mock_full_explanation()
        )
    return SimpleNamespace(
        stop_reason="tool_use",
        content=[
...
class Messages:
    def __init__(self):
        ...
        self.parse = messages_parse_mock
